...
class TimelineEventService {
  ...
  protected function getEventDate($event) {
    if ($event instanceof NodeInterface) {
      ...
      $date_fields = [
        ...
        'field_this_day_in_history_date_2',   // Secondary TDIH field  
        ...
        'field_publication_date_archive',      // Publication dates
        'field_archive_publication_date',      // Archive dates
  ...
  protected function extractDateFromText($text) {
    ...
    $patterns = [
      '/(\d{1,2})(?:st|nd|rd|th)?\s+(?:of\s+)?([A-Za-z]+)\s+(\d{4})/',  // 25th of December 1994
      '/([A-Za-z]+)\s+(\d{1,2})(?:st|nd|rd|th)?,?\s+(\d{4})/',          // December 25th, 1994
      '/(\d{4})\s*[-–—]\s*(\d{4})/',                                     // 1994-1996
      '/(\d{4})/',                                                       // Just a year
    ];
