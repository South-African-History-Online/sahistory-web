...
(() => {
  ...
  function initViewToggle() {
    ...
    if (
      !resultsContainer.classList.contains('list-view') &&
      !resultsContainer.classList.contains('grid-view')
    ) {
  ...
  function generateRelatedSearches() {
    ...
    function findRelatedTerms(query) {
      ...
      return [...new Set(terms)].filter((term) => term.toLowerCase() !== queryLower).slice(0, 5); // Limit to 5 terms
    }
