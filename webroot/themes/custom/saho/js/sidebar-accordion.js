...
(function ($, Drupal, once) {
  ...
  Drupal.behaviors.sahoSidebarAccordion = {
    attach: function (context, settings) {
      once('saho-sidebar-accordion', '.saho-accordion', context).forEach(function (element) {
  ...
  function initializeAccordion($accordion) {
    ...
    const $collapses = $accordion.find('.saho-accordion-collapse');
    ...
    if ($firstButton.length) {
      const $firstTarget = $($firstButton.attr('data-target') || $firstButton.attr('aria-controls'));
  ...
  function toggleAccordionItem($button, $target, $accordion) {
    ...
    if (isExpanded) {
      ...
      $otherButtons.each(function() {
        ...
        const $otherTarget = $($otherButton.attr('data-target') || $otherButton.attr('aria-controls'));
  ...
  Drupal.behaviors.sahoConvertBootstrapAccordion = {
    attach: function (context, settings) {
      once('saho-convert-accordion', '.accordion', context).forEach(function (element) {
        const $accordion = $(element);
