...
(($, Drupal, once) => {
  ...
  Drupal.behaviors.sahoUrlTruncation = {
    attach: (context, _settings) => {
      ...
      once('saho-url-truncation', selectors, context).forEach((element) => {
        const $link = $(element);
        ...
        if (
          (isUrl(linkText) && linkText.length > 50) ||
          (fullUrl && isUrl(fullUrl) && fullUrl.length > 50)
        ) {
   ...
   */
  function enhanceUrlLink($link, _fullUrl, linkText) {
  ...
  function copyToClipboard(text) {
    if (navigator.clipboard && window.isSecureContext) {
      ...
      navigator.clipboard.writeText(text).catch((_err) => {
   ...
   */
  function _createTooltip($link, fullUrl) {
    let tooltipTimeout;

    $link.on('mouseenter focus', () => {
