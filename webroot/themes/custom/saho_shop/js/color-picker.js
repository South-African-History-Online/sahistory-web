/**
 * @file
 * Provides UI/UX progressive enhancements on SahoShop's theme settings by
...
((Drupal, settings, once) => {
  const colorSchemeOptions = settings.saho_shop.colorSchemes;
  ...
  function synchronizeInputs(changedInput, inputToSync) {
    ...
    changedInput.setAttribute('data-saho_shop-custom-color', changedInput.value);
    inputToSync.setAttribute('data-saho_shop-custom-color', changedInput.value);
  ...
  function setColorScheme({ target }) {
    ...
    if (selectedColorScheme) {
      ...
      document
        .querySelectorAll(`input[data-saho_shop-custom-color]`)
        .forEach((input) => {
          input.value = input.getAttribute('data-saho_shop-custom-color');
        });
    }
  }
  ...
  /**
   * Initializes SahoShop theme-settings color picker.
  ...
  function initColorPicker(textInput) {
    ...
    colorInput.setAttribute(
      'data-saho_shop-custom-color',
      textInput.getAttribute('data-saho_shop-custom-color'),
    );
    ...
    // Adjust CSS.
    textInput.style.verticalAlign = 'bottom';
  }

  /**
   * SahoShop Color Picker behavior.
   *
   * @type {Drupal~behavior}
   * @prop {Drupal~behaviorAttach} attach
   *   Initializes color picker fields.
   */
  Drupal.behaviors.saho_shopColorPicker = {
    attach: () => {
      const colorSchemeSelect = once(
        'saho_shop-color-picker',
        '[data-drupal-selector="edit-color-scheme"]',
      );
      ...
      const colorTextInputs = once(
        'saho_shop-color-picker',
        '[data-drupal-selector="saho_shop-color-picker"] input[type="text"]',
      );
