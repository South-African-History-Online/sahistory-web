...
1. **List of failing product IDs** (or titles)
2. **List of working product IDs** (or titles)
3. **Output of the "Get Exact Error" command** for one failing product
4. **Output of "Which Template" debug** (with Twig debug enabled)
5. **URL of one failing product**

Example:
```
WORKING: Product 5 (Africa in Today's World)
FAILING: Product 27 (African National Congress...)
ERROR: Object of type Taxonomy\Term cannot be printed
TEMPLATE: commerce-product--publication--full.html.twig
```

---

## 🔧 Quick Fix: Check All Templates

```bash
# Find all product templates
find webroot/themes/custom/saho_shop/templates -name "*commerce-product*.twig"

# If you find multiple templates, check each one for the bug:
grep -n "product\\.field_" webroot/themes/custom/saho_shop/templates/commerce/*.twig

# Any line using product.field_* instead of content.field_* is a bug!
```
